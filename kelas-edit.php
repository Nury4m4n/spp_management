    ...
    <div class="col-6">
      ...
      <form action="kelas-update.php" method="post">
        ...
        <div class="input-group mb-1 input-sm">
          ...
          <input type="text" name="nama_kelas" class="form-control form-control-sm" value="<?= $data['nama_kelas']; ?>"
          ...
          <select name="id_kompetensi" class="form-control form-control-chosen" required>
            <?php
						...
						while ($d = mysqli_fetch_array($query)) {
							$id = $d['id_kompetensi']; ?>
            <option value="<?= $id; ?>" <?php if ($id == $id_kompetensi) {
																						echo 'selected="selected"';
																					} ?>>
              <?= $d['nama_kompetensi_keahlian']; ?></option>
            <?php
						} ?>
          </select>
        </div>
        ...
        <div class="input-group mb-1 input-sm">
          ...
          <input type="text" name="wali_kelas" class="form-control form-control-sm" value="<?= $data['wali_kelas']; ?>"
            autofocus autocomplete="off" required>
        </div>

        <div class="input-group mb-1">
          <button type="submit" class="btn btn-success btn-sm"><i class="fas fa-save"></i> Update</button> | <a
            href="kelas.php" class="btn btn-sm btn-warning"><i class="fas fa-redo"></i> Cancel</a>
        </div>
      </form>
    </div>
  </div>
